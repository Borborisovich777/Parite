...
export const BottomNav: React.FC<BottomNavProps> = ({
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-[#121418]/95 backdrop-blur border-t border-slate-800 flex justify-around items-center px-2 z-30 shadow-[0_-8px_28px_rgba(0,0,0,0.45)] h-[68px] shrink-0">
      {tabs.map(tab => {
          ...
          >
            <div className={`relative p-1.5 rounded-2xl transition-transform duration-150 active:scale-95 ${
              ...
              {'badge' in tab && tab.badge > 0 ? (
                <span className="absolute -top-0.5 -right-0.5 bg-rose-500 text-white font-mono font-bold text-[9px] w-4.5 h-4.5 rounded-full flex items-center justify-center border border-[#1a1d23] animate-pulse">
                  {tab.badge}
                </span>
              ) : null}
            </div>
